...
const getAccount = async (web3) => (async (res) => typeof res === 'string'
    ? res
    : web3.eth
        .getAccounts()
...
const execute = async ({ contract, method, args, mutation, client, padEnd, }) => {
...
const createSchemaCaller = (contract) => always(execute({ contract, method: 'schema' }).then((result) => JSON.parse(result
    .split(`'`)
    .map((v) => ({ v, s: /[[\],:]/.test(v) }))
...
const watchEvent = async ({ contract, resolver, fromBlock = 0, }) => new Promise((resolve, reject) => {
...
const createAuthenticateCaller = (contract, client) => async (propertyAddress, args, { metricsFactory }) => new Promise((resolve, reject) => {
    // eslint-disable-next-line functional/no-expression-statement
    execute({
        contract,
        method: 'authenticate',
...
const createAuthorCaller = (contract) => always(execute({ contract, method: 'author' }));
...
const createNameCaller = (contract) => always(execute({ contract, method: 'name' }));

const createSymbolCaller = (contract) => always(execute({ contract, method: 'symbol' }));

const createTotalSupplyCaller = (contract) => always(execute({ contract, method: 'totalSupply' }));

const createDecimalsCaller = (contract) => always(execute({ contract, method: 'decimals' }));
...
const createBalanceOfCaller = (contract) => async (address) => execute({ contract, method: 'balanceOf', args: [address] });
...
const createCreateAndAuthenticateCaller = (contract, client) => async (name, symbol, marketAddress, args, { metricsFactory }) => {
...
const createGetPropertyValueCaller = (contract) => async (address) => execute({ contract, method: 'getPropertyValue', args: [address] });
...
const createGetAllValueCaller = (contract) => always(execute({ contract, method: 'getAllValue' }));
...
const createCapCaller = (contract) => always(execute({ contract, method: 'cap' }));
...
const createGetRewardsAmountCaller = (contract) => async (address) => execute({ contract, method: 'getRewardsAmount', args: [address] });
...
const createTokenCaller = (contract) => always(execute({ contract, method: 'token' }));

const createAllocatorCaller = (contract) => always(execute({ contract, method: 'allocator' }));

const createAllocatorStorageCaller = (contract) => always(execute({ contract, method: 'allocatorStorage' }));

const createLockupCaller = (contract) => always(execute({ contract, method: 'lockup' }));

const createLockupStorageCaller = (contract) => always(execute({ contract, method: 'lockupStorage' }));

const createMarketFactoryCaller = (contract) => always(execute({ contract, method: 'marketFactory' }));

const createMarketGroupCaller = (contract) => always(execute({ contract, method: 'marketGroup' }));

const createMetricsFactoryCaller = (contract) => always(execute({ contract, method: 'metricsFactory' }));

const createMetricsGroupCaller = (contract) => always(execute({ contract, method: 'metricsGroup' }));

const createPolicyCaller = (contract) => always(execute({ contract, method: 'policy' }));

const createPropertyFactoryCaller = (contract) => always(execute({ contract, method: 'propertyFactory' }));

const createPropertyGroupCaller = (contract) => always(execute({ contract, method: 'propertyGroup' }));

const createWithdrawCaller = (contract) => always(execute({ contract, method: 'withdraw' }));

const createWithdrawStorageCaller = (contract) => always(execute({ contract, method: 'withdrawStorage' }));

const createPolicyFactoryCaller = (contract) => always(execute({ contract, method: 'policyFactory' }));

const createPolicySetCaller = (contract) => always(execute({ contract, method: 'policySet' }));

const createPolicyGroupCaller = (contract) => always(execute({ contract, method: 'policyGroup' }));
...
const createTotalAuthenticatedPropertiesCaller = (contract) => always(execute({ contract, method: 'totalAuthenticatedProperties' }));
