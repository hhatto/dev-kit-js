...
var asyncRay = require('async-ray');
...
const getAccount = async web3 => (async res => typeof res === 'string' ? res : web3.eth.getAccounts().then(([account]) => map.set(web3, account).get(web3)))(map.get(web3));

const txPromisify = async tx => {
  return new Promise((resolve, reject) => {
    // eslint-disable-next-line functional/no-expression-statement
    tx.on('confirmation', (_, receipt) => receipt.status ? resolve(receipt) : reject(new Error('An error occurred in the transaction.'))).on('error', err => reject(err)).catch(err => reject(err));
  });
};

const pad = (args, index) => (fn => fn([], args[0], 0, fn))((arr, v, i, fn) => i < index ? fn(arr.concat(v !== null && v !== void 0 ? v : ''), args[i + 1], i + 1, fn) : arr);
...
const execute = async ({
  ...
  const x = a ? m(...a) : m();
  return mutation === true ? txPromisify(x.send({
    from: await getAccount(client)
  })).then(receipt => receipt) : client ? x.call({
    from: await getAccount(client)
  }) : x.call();
};
/**
...
const createSchemaCaller = contract => always(execute({
  contract,
  method: 'schema'
}).then(result => JSON.parse(result.split(`'`).map(v => ({
  v,
  s: /[[\],:]/.test(v)
})).reduce((a, c) => `${a}${c.s ? c.v : `"${c.v}"`}`, ''))));

const createVoteCaller = (contract, client) => async (propertyAddress, agree) => execute({
  contract,
  method: 'vote',
  mutation: true,
  client,
...
const createBehaviorCaller = contract => always(execute({
  contract,
  method: 'behavior'
}));
...
const watchEvent = async ({
  ...
  const {
    events
  } = contract; // eslint-disable-next-line functional/no-expression-statement

  events.allEvents({
    fromBlock,
    toBlock: 'latest'
  }, (err, e) => err ? reject(err) : resolver(e).then(res => res ? resolve(e) : undefined));
});
...
const waitForCreateMetrics = async (client, propertyAddress, metricsFactoryAddress) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    watchEvent({
      ...
      resolver: async e => (metricsAddress => metricsAddress ? getMetricsProperty(metricsAddress, client).then(property => property === propertyAddress ? true : false).catch(reject) : false)(e.event === 'Create' ? e.returnValues._metrics : undefined)
    }).then(res => resolve(res.returnValues._metrics)).catch(reject);
  });
};

const createAuthenticateCaller = (contract, client) => async (propertyAddress, args, {
  metricsFactory
}) => new Promise((resolve, reject) => {
  // eslint-disable-next-line functional/no-expression-statement
  execute({
    ...
    padEnd: 6
  }).catch(reject); // eslint-disable-next-line functional/no-expression-statement

  waitForCreateMetrics(client, propertyAddress, metricsFactory).then(resolve).catch(reject);
}); // eslint-disable-next-line @typescript-eslint/prefer-readonly-parameter-types


const createMarketContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...marketAbi], address, { ...options
  });
  return {
...
const createAuthorCaller = contract => always(execute({
  contract,
  method: 'author'
}));

const createChangeAuthorCaller = (contract, client) => async nextAuther => execute({
  contract,
  method: 'changeAuthor',
  mutation: true,
  client,
...
const createChangeNameCaller = (contract, client) => async nextName => execute({
  contract,
  method: 'changeName',
  mutation: true,
  client,
...
const createChangeSymbolCaller = (contract, client) => async nextSymbol => execute({
  contract,
  method: 'changeSymbol',
  mutation: true,
  client,
...
const createTransferCaller = (contract, client) => async (to, value) => execute({
  contract,
  method: 'transfer',
  mutation: true,
  client,
...
const createNameCaller = contract => always(execute({
  contract,
  method: 'name'
}));

const createSymbolCaller = contract => always(execute({
  contract,
  method: 'symbol'
}));

const createTotalSupplyCaller = contract => always(execute({
  contract,
  method: 'totalSupply'
}));

const createDecimalsCaller = contract => always(execute({
  contract,
  method: 'decimals'
}));

const createTransferFromCaller = (contract, client) => async (from, to, value) => execute({
  contract,
  method: 'transferFrom',
  mutation: true,
  client,
...
const createBalanceOfCaller = contract => async address => execute({
  contract,
  method: 'balanceOf',
  args: [address]
});

const createApproveCaller = (contract, client) => async (to, value) => execute({
  contract,
  method: 'approve',
  mutation: true,
  client,
...
const createAllowanceCaller = contract => async (from, to) => execute({
  contract,
  method: 'allowance',
  args: [from, to]
});

const createPropertyContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...propertyAbi], address, { ...options
  });
  return {
...
const createCreatePropertyCaller = (contract, client) => async (name, symbol, author) => execute({
  contract,
  method: 'create',
  ...
  events
}) => events.Create.returnValues._property);

const createCreateAndAuthenticateCaller = (contract, client) => async (name, symbol, marketAddress, args, {
  metricsFactory
}) => {
  ...
  const property = transaction.events.Create.returnValues._property;
  return {
    property,
    transaction,
...
const createPropertyFactoryContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...propertyFactoryAbi], address, { ...options
  });
  return {
...
const createCalculateMaxRewardsPerBlockCaller = contract => always(execute({
  contract,
  method: 'calculateMaxRewardsPerBlock'
}));

const createAllocatorContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...allocatorAbi], address, { ...options
  });
  return {
...
const createGetValueCaller = contract => async (propertyAddress, accountAddress) => execute({
  contract,
  method: 'getValue',
  args: [propertyAddress, accountAddress]
});

const createGetPropertyValueCaller = contract => async address => execute({
  contract,
  method: 'getPropertyValue',
  args: [address]
});

const createWithdrawCaller$2 = (contract, client) => async (propertyAddress, amount) => execute({
  contract,
  method: 'withdraw',
  mutation: true,
  client,
...
const createCalculateWithdrawableInterestAmountCaller = contract => async (propertyAddress, account) => execute({
  contract,
  method: 'calculateWithdrawableInterestAmount',
  args: [propertyAddress, account]
});

const createGetAllValueCaller = contract => always(execute({
  contract,
  method: 'getAllValue'
}));

const createGetStorageWithdrawalStatusCaller = contract => async (propertyAddress, accountAddress) => execute({
  contract,
  method: 'getStorageWithdrawalStatus',
  args: [propertyAddress, accountAddress]
});

const createCalculateCumulativeHoldersRewardAmountCaller = contract => async propertyAddress => execute({
  contract,
...
const createCalculateCumulativeRewardPricesCaller = contract => always(execute({
  contract,
  method: 'calculateCumulativeRewardPrices'
}).then(r => arrayify(r)));

const createCalculateRewardAmountCaller = contract => async propertyAddress => execute({
  contract,
  method: 'calculateRewardAmount',
  args: [propertyAddress]
}).then(r => arrayify(r));

const createCapCaller = contract => always(execute({
  contract,
  method: 'cap'
}));

const createLockupContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...lockupAbi], address, { ...options
  });
  return {
    ...
    withdraw: createWithdrawCaller$2(contractClient, client),
...
const createDepositCaller = (contract, client) => async (to, value) => execute({
  contract,
  method: 'deposit',
  mutation: true,
  client,
...
const createDevContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...devAbi], address, { ...options
  });
  return {
    totalSupply: createTotalSupplyCaller(contractClient),
    balanceOf: createBalanceOfCaller(contractClient),
...
const createWithdrawCaller$1 = (contract, client) => async propertyAddress => execute({
  contract,
  method: 'withdraw',
  mutation: true,
  client,
...
const createGetRewardsAmountCaller = contract => async address => execute({
  contract,
  method: 'getRewardsAmount',
  args: [address]
});

const createCalculateWithdrawableAmountCaller = contract => async (propertyAddress, accountAddress) => execute({
  contract,
  method: 'calculateWithdrawableAmount',
  args: [propertyAddress, accountAddress]
});

const createBulkWithdrawCaller = (contract, client) => async propertyAddresses => execute({
  contract,
  method: 'bulkWithdraw',
  mutation: true,
  client,
...
const calculateRewardAmountCaller = contract => async (propertyAddress, accountAddress) => execute({
  contract,
  method: 'calculateRewardAmount',
  args: [propertyAddress, accountAddress]
}).then(r => arrayify(r));

const createWithdrawContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...withdrawAbi], address, { ...options
  });
  return {
    withdraw: createWithdrawCaller$1(contractClient, client),
...
const createTokenCaller = contract => always(execute({
  contract,
  method: 'token'
}));

const createAllocatorCaller = contract => always(execute({
  contract,
  method: 'allocator'
}));

const createAllocatorStorageCaller = contract => always(execute({
  contract,
  method: 'allocatorStorage'
}));

const createLockupCaller = contract => always(execute({
  contract,
  method: 'lockup'
}));

const createLockupStorageCaller = contract => always(execute({
  contract,
  method: 'lockupStorage'
}));

const createMarketFactoryCaller = contract => always(execute({
  contract,
  method: 'marketFactory'
}));

const createMarketGroupCaller = contract => always(execute({
  contract,
  method: 'marketGroup'
}));

const createMetricsFactoryCaller = contract => always(execute({
  contract,
  method: 'metricsFactory'
}));

const createMetricsGroupCaller = contract => always(execute({
  contract,
  method: 'metricsGroup'
}));

const createPolicyCaller = contract => always(execute({
  contract,
  method: 'policy'
}));

const createPropertyFactoryCaller = contract => always(execute({
  contract,
  method: 'propertyFactory'
}));

const createPropertyGroupCaller = contract => always(execute({
  contract,
  method: 'propertyGroup'
}));

const createWithdrawCaller = contract => always(execute({
  contract,
  method: 'withdraw'
}));

const createWithdrawStorageCaller = contract => always(execute({
  contract,
  method: 'withdrawStorage'
}));

const createPolicyFactoryCaller = contract => always(execute({
  contract,
  method: 'policyFactory'
}));

const createPolicySetCaller = contract => always(execute({
  contract,
  method: 'policySet'
}));

const createPolicyGroupCaller = contract => always(execute({
  contract,
  method: 'policyGroup'
}));

const createRegistryContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...addressConfigAbi], address, { ...options
  });
  return {
...
const createHoldersShareCaller = contract => async (amount, lockups) => execute({
  contract,
  method: 'holdersShare',
  args: [amount, lockups]
});

const createRewardsCaller = contract => async (lockups, assets) => execute({
  contract,
  method: 'rewards',
  args: [lockups, assets]
});

const createAuthenticationFeeCaller = contract => async (assets, propertyAssets) => execute({
  contract,
  method: 'authenticationFee',
  args: [assets, propertyAssets]
});

const createMarketApprovalCaller = contract => async (agree, opposite) => execute({
  contract,
  method: 'marketApproval',
  args: [agree, opposite]
});

const createPolicyApprovalCaller = contract => async (agree, opposite) => execute({
  contract,
  method: 'policyApproval',
  args: [agree, opposite]
});

const createMarketVotingBlocksCaller = contract => always(execute({
  contract,
  method: 'marketVotingBlocks'
}));

const createPolicyVotingBlocksCaller = contract => always(execute({
  contract,
  method: 'policyVotingBlocks'
}));

const createShareOfTreasuryCaller = contract => async supply => execute({
  contract,
  method: 'shareOfTreasury',
  args: [supply]
});

const createTreasuryCaller = contract => always(execute({
  contract,
  method: 'treasury'
}));

const createCapSetterCaller = contract => always(execute({
  contract,
  method: 'capSetter'
}));

const createPolicyContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...policyAbi], address, { ...options
  });
  return {
...
const createVotingCaller = contract => async policyAddress => execute({
  contract,
  method: 'voting',
  args: [policyAddress]
});

const createIsGroupCaller = contract => async policyAddress => execute({
  contract,
  method: 'isGroup',
  args: [policyAddress]
});

const createPolicyGroupContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...policyGroupAbi], address, { ...options
  });
  return {
...
const createPropertyCaller = contract => always(execute({
  contract,
  method: 'property'
}));

const createMarketCaller = contract => always(execute({
  contract,
  method: 'market'
})); // eslint-disable-next-line @typescript-eslint/prefer-readonly-parameter-types


const createMetricsContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...metricsAbi], address, { ...options
  });
  return {
...
const createConvergePolicyCaller = (contract, client) => async currentPolicyAddress => execute({
  contract,
...
const createCreateCaller = (contract, client) => async newPolicyAddress => execute({
  contract,
  method: 'create',
  mutation: true,
  client,
...
const createForceAttachCaller = (contract, client) => async policy => execute({
  contract,
  method: 'forceAttach',
  mutation: true,
  client,
...
const createPolicyFactoryContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...policyFactoryAbi], address, { ...options
  });
  return {
...
const createDevkitContract = client => ({
...
const contractFactory = provider => {
  const client = new Web3__default['default'](provider);
...
const createGetVotablePolicy = policyGroup => {
  const fetcher = always(bent__default['default'](DEV_GRAPHQL_ENDPOINT, 'POST', 'json')('/', {
    query: `{
  ...
  const checker = async policy => policyGroup.isGroup(policy); // eslint-disable-next-line functional/functional-parameters


  return async () => {
    const policies = await fetcher().then(({
      data
    }) => data.policy_factory_create);
    const policyAddresses = policies.map(({
      policy_address
    }) => policy_address);
    const notPolicyIndex = await asyncRay.aFindIndex(policyAddresses, async policy => not(await checker(policy)));
...
const TEAM_WALLET_ADDRESSES = ['0x33b5043442979D2226E9a6389F7201932D11e448', '0xA47c73A77d358A985157034A2338fAB7742B107E', '0x0dAb082C2f2CD7C6C2a9335b69d0B2aB8121178D', '0x93d7A66E10b6a8a5a00313bC68F0FB234c8eB06D', '0x6B18fDa007ec96E187e5CF89D1873B9F75D5293D'];

const falsyOrZero = num => num ? num : 0;

const toNaturalBasis = new BigNumber(10).pow(18);

const toNaturalNumber = num => new BigNumber(falsyOrZero(num)).div(toNaturalBasis);

const getEthPrice = always(bent__default['default'](THEGRAPH_UNISWAP_ENDPOINT, 'POST', 'json')('', {
  query: `{ bundle(id: 1) { id ethPrice } }`,
  variables: null
}).then(r => r));

const getDevEthPrice = async devkit => {
  var _addresses$eth$main;

  const devContractAddress = await devkit.registry((_addresses$eth$main = addresses.eth['main']) === null || _addresses$eth$main === void 0 ? void 0 : _addresses$eth$main.registry)['token']();
  return bent__default['default'](THEGRAPH_UNISWAP_ENDPOINT, 'POST', 'json')('', {
...
const getDevPrice = async devkit => {
  var _devEthPrice$token;

  const [{
    data: ethPrice
  }, {
    data: devEthPrice
  }] = await Promise.all([getEthPrice(), getDevEthPrice(devkit)]);
  const devPrice = ethPrice && Number(ethPrice === null || ethPrice === void 0 ? void 0 : ethPrice.bundle.ethPrice) * Number((devEthPrice === null || devEthPrice === void 0 ? void 0 : (_devEthPrice$token = devEthPrice.token) === null || _devEthPrice$token === void 0 ? void 0 : _devEthPrice$token.derivedETH) || '0');
  return new BigNumber(devPrice || '0');
};

const getTotalCap = async devkit => {
  var _addresses$eth$main2;

  const devContractAddress = await devkit.registry((_addresses$eth$main2 = addresses.eth['main']) === null || _addresses$eth$main2 === void 0 ? void 0 : _addresses$eth$main2.registry)['token']();
...
const getCirculatingSupply = async devkit => {
  var _addresses$eth$main3;

  const devContractAddress = await devkit.registry((_addresses$eth$main3 = addresses.eth['main']) === null || _addresses$eth$main3 === void 0 ? void 0 : _addresses$eth$main3.registry)['token']();
...
const getTotalStakingAmount = async devkit => {
  var _addresses$eth$main4;

  const devContractAddress = await devkit.registry((_addresses$eth$main4 = addresses.eth['main']) === null || _addresses$eth$main4 === void 0 ? void 0 : _addresses$eth$main4.registry)['lockup']();
  const totalStakingAmount = await devkit.lockup(devContractAddress).getAllValue();
...
const getAPY = async devkit => {
  var _addresses$eth$main5, _addresses$eth$main6;

  const allocatorContractAddress = await devkit.registry((_addresses$eth$main5 = addresses.eth['main']) === null || _addresses$eth$main5 === void 0 ? void 0 : _addresses$eth$main5.registry)['allocator']();
  const maxRewards = await devkit.allocator(allocatorContractAddress).calculateMaxRewardsPerBlock();
  const totalStakingAmount = await getTotalStakingAmount(devkit);
  const policyContractAddress = await devkit.registry((_addresses$eth$main6 = addresses.eth['main']) === null || _addresses$eth$main6 === void 0 ? void 0 : _addresses$eth$main6.registry)['policy']();
  const holdersShare = await devkit.policy(policyContractAddress).holdersShare(maxRewards, totalStakingAmount.toFixed());
  ...
  const creatorAPY = new BigNumber(holdersShare).times(year).div(totalStakingAmount);
  return {
    stakerAPY,
    creatorAPY
  };
...
const getTotalSupply = async devkit => {
  var _addresses$eth$main7;

  const devContractAddress = await devkit.registry((_addresses$eth$main7 = addresses.eth['main']) === null || _addresses$eth$main7 === void 0 ? void 0 : _addresses$eth$main7.registry)['token']();
  const totalSupply = await devkit.dev(devContractAddress).totalSupply();
  return totalSupply;
};

const getSupplyGrowth = async devkit => {
  var _addresses$eth$main8;

  const allocatorContractAddress = await devkit.registry((_addresses$eth$main8 = addresses.eth['main']) === null || _addresses$eth$main8 === void 0 ? void 0 : _addresses$eth$main8.registry)['allocator']();
  const maxRewards = await devkit.allocator(allocatorContractAddress).calculateMaxRewardsPerBlock();
  const totalSupply = toNaturalNumber(await getTotalSupply(devkit));
  const year = new BigNumber(2102400);
  const annualSupplyGrowthRatio = toNaturalNumber(maxRewards).times(year).div(totalSupply);
  return annualSupplyGrowthRatio;
};

const getAssetOnboarded = always(bent__default['default'](DEV_GRAPHQL_ENDPOINT, 'POST', 'json')('/', {
  query: `{ property_factory_create_aggregate(
			where: { authentication: { authentication_id: { _is_null: false } } }
		) { aggregate { count } } }`,
  variables: null
}).then(r => r.data.property_factory_create_aggregate.aggregate.count));
...
const getStats = async httpProviderEndpoint => {
  const web3 = new Web3__default['default'](httpProviderEndpoint);
  const devkit = createDevkitContract(web3);
  const [devPrice, totalCap, marketCap, stakingRatio, stakingAmount, {
    stakerAPY,
    creatorAPY
  }, annualSupplyGrowthRatio, assetOnboarded] = await Promise.all([getDevPrice(devkit), getTotalCap(devkit), getMarketCap(devkit), getStakingRatio(devkit), getTotalStakingAmount(devkit), getAPY(devkit), getSupplyGrowth(devkit), getAssetOnboarded()]);
...
const createTotalAuthenticatedPropertiesCaller = contract => always(execute({
  contract,
  method: 'totalAuthenticatedProperties'
})); // eslint-disable-next-line @typescript-eslint/prefer-readonly-parameter-types


const createMetricsGroupContract = client => (address, options) => {
  const contractClient = new client.eth.Contract([...metricsGroupAbi], address, { ...options
  });
  return {
...
exports.createAllocatorContract = createAllocatorContract;
exports.createDevContract = createDevContract;
exports.createDevkitContract = createDevkitContract;
exports.createLockupContract = createLockupContract;
exports.createMarketContract = createMarketContract;
exports.createMetricsContract = createMetricsContract;
exports.createMetricsGroupContract = createMetricsGroupContract;
exports.createPolicyContract = createPolicyContract;
exports.createPolicyFactoryContract = createPolicyFactoryContract;
exports.createPolicyGroupContract = createPolicyGroupContract;
exports.createPropertyContract = createPropertyContract;
exports.createPropertyFactoryContract = createPropertyFactoryContract;
exports.createRegistryContract = createRegistryContract;
exports.createWithdrawContract = createWithdrawContract;
exports.utils = utils;
